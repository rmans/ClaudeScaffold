...
High-level principles that guide all input design decisions. Why we make the input choices we do.

## Principles

<!-- Define the guiding principles for input design. Examples:

1. **Actions, not keys.** Code references semantic actions, never raw keys.
2. **Device-agnostic.** All gameplay must be playable on keyboard and gamepad.
3. **Remappable by default.** Players can remap all gameplay actions.
4. **Prompts match device.** Button prompts reflect the active input device.
