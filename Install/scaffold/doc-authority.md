...
| **Revision logs** | Drift correction | Systems, references, engine docs (Rank 5–6, 10) | Auto-updates fix stale references and alignments in ranked docs. Escalations require user decisions (which may produce ADRs). |
| **Code review logs** | Code + doc correction | Code files + engine docs (Rank 10) | Accepted findings directly edit code. Engine doc drift detected during review feeds `/scaffold-revise-engine`. |
...
| Doc | What | Why | Who reads | Who writes | When created | How it's used |
|-----|------|-----|-----------|------------|-------------|---------------|
| **style-guide.md** | Visual identity, rendering style, lighting model, animation style, iconography | Defines what the game LOOKS like | Art skills, UI skills, engine UI doc | bulk-seed-style | Step 5 | Visual decisions that all presentation must follow. |
| **color-system.md** | Color palette, semantic colors, accessibility rules | Defines the color LANGUAGE | Art skills, UI skills | bulk-seed-style | Step 5 | Every color choice traces back here. |
| **ui-kit.md** | UI component definitions, panel patterns, layout rules, theme conventions | Defines how UI is STRUCTURED | Engine UI doc, task implementations | bulk-seed-style | Step 5 | Governs all UI implementation. |
...
| **decisions/playtest-feedback/PF-###** | Playtester observations and patterns | Records PLAYER BEHAVIOR | Patterns (3+ reports) feed revise-phases and revise-slices as scope input. |
...
| **decisions/code-review/** | Adversarial code review session logs | Records CODE QUALITY findings | Engine doc drift detected here feeds revise-engine. |
