#!/usr/bin/env python3
"""ClaudeScaffold installer — copies the scaffold overlay into a target project."""
...
def main():
    ...
    # Locate Install/ relative to this script
    script_dir = Path(__file__).resolve().parent
    install_dir = script_dir / "Install"

    if not install_dir.is_dir():
        print(f"ERROR: Cannot find Install/ directory at {install_dir}", file=sys.stderr)
        print("Run this script from the ClaudeScaffold repository root.", file=sys.stderr)
        sys.exit(1)
    ...
    if src_skills_dir.is_dir():
        for skill_dir in sorted(src_skills_dir.iterdir()):
            ...
            if skill_dir.name in EXCLUDE_DIRS:
                continue
            dst_skill = dst_skills_dir / skill_dir.name
